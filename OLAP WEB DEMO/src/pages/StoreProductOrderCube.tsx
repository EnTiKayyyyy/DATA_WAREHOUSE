...
const StoreProductOrderCube: React.FC = () => {
  ...
  // State for pivot
  const [rowDimension, setRowDimension] = useState<string>('StoreID');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.getStoreProductOrderCube();
        setData(response);
        setLoading(false);
      } catch (err) {
        setError('Failed to fetch store product order data');
        setLoading(false);
      }
    };
  ...
  const dimensions = [
    {
      name: 'StoreID',
      values: getDimensionValues('StoreID').map(String)
    },
    {
      name: 'CityName',
  ...
  const getColumns = () => {
    const baseColumns = [
      ...
      { key: 'TotalQuantityOrdered', label: 'Total Quantity' }
    ];
  ...
  return (
    <div>
      <div className="flex items-center mb-6">
        <div className="p-2 rounded-md bg-red-100 dark:bg-red-900 mr-3">
          <Store className="h-6 w-6 text-red-600 dark:text-red-400" />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Store Product Order Cube</h1>
          <p className="text-gray-600 dark:text-gray-400">
            Analyze store performance by products and orders
          </p>
        </div>
      </div>
      ...
      {/* Pivot Control */}
      <PivotControl
        dimensions={['StoreID', 'CityName', 'ProductDescription']}
        rows={rowDimension}
      ...
      <DataTable
        columns={getColumns()}
        data={filteredData}
        title="Store Product Order Analysis"
      />
    </div>
  );
};

export default StoreProductOrderCube;