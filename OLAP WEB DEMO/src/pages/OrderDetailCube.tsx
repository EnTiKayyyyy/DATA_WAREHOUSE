...
const OrderDetailCube: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.getOrderDetailCube();
        setData(response);
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div>
      <div className="flex items-center mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Order Detail Cube</h1>
          <p className="text-gray-600 dark:text-gray-400">
            Analyze detailed order information by product, customer, and time
          </p>
        </div>
      </div>
...
export default OrderDetailCube;